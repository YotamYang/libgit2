...
static int on_header_ready(http_subtransport *t)
{
	...
	if (!strcasecmp("Content-Length", git_buf_cstr(name))) {
		if (!t->content_length) {
		...
		git_vector_insert(&t->proxy.auth_challenges, dup);
	}
	else if (!strcasecmp("WWW-Authenticate", git_buf_cstr(name))) {
		char *dup = git__strdup(git_buf_cstr(value));
		GITERR_CHECK_ALLOC(dup);

		git_vector_insert(&t->server.auth_challenges, dup);
	}
...
{
	gitno_connection_data *url;
	git_stream *proxy_stream = NULL, *stream;
	git_transport_certificate_check_cb cert_cb;
	void *cb_payload;
	int error;
	...
	git_smart_service_t action)
{
	...
	/*
	 * Callers should provide a non-NULL url for the first connection
	 * to a server.  Subsequent calls should pass a NULL url here to
	 * reuse the existing one.
	 */
	if (url) {
		assert(!t->server.url.host && !t->server.url.port && !t->server.url.path);

		if ((ret = gitno_connection_data_from_url(&t->server.url, url, NULL)) < 0)
			return ret;
